			...
			<div class="panel-body">
				  <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form" class="form-horizontal">
						...
						<div class="form-group">
							...
							<div class="col-sm-3">
							  <select name="duitku_va_cimb_status" id="input-mode" class="form-control">
								<?php $options = array('1' => $text_enabled, '0' => $text_disabled) ?>
								<?php foreach ($options as $key => $value): ?>
								  <option value="<?php echo $key ?>" <?php if ($key == $duitku_va_cimb_status) echo 'selected' ?> ><?php echo $value ?></option>
								<?php endforeach ?>
							  </select>
							</div>
						</div> 
						...
						<div class="form-group v2_settings sensitive required">
							...
							<div class="col-sm-3">
								<select name="duitku_va_cimb_environment" id="input-mode" class="form-control">
									<?php $options = array('development' => 'Sandbox', 'production' => 'Production') ?>
									<?php foreach ($options as $key => $value): ?>
									<option value="<?php echo $key ?>" <?php if ($key == $duitku_va_cimb_environment) echo 'selected' ?> ><?php echo $value ?></option>
									<?php endforeach ?>
								</select>
							</div>
							<div class="col-sm-7">
					...
					<div class="form-group">
						<label class="col-sm-2 control-label" for="input-merchant-id"><?php echo $entry_geo_zone; ?></label>
							<div class="col-sm-3">
								...
								<?php foreach ($geo_zones as $geo_zone) { ?>
									<?php if ($geo_zone['geo_zone_id'] == $duitku_va_cimb_geo_zone_id) { ?>
						...
						<div class="form-group">
							...
							<div class="col-sm-1">
							  <input size="1" type="text" name="duitku_va_cimb_sort_order" value="<?php echo $duitku_va_cimb_sort_order; ?>" class="form-control" />
							</div>
						</div>
						<div>
