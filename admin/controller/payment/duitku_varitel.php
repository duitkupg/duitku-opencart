...
class ControllerPaymentDuitkuVaritel extends Controller {
  ...
  public function index() {
    ...
    if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
      $this->model_setting_setting->editSetting('duitku_varitel', $this->request->post);
    ...
    $language_entries = array(
      ...
      'entry_status',
      'entry_sort_order',                              
      'entry_duitku_varitel_success_mapping',
      'entry_duitku_varitel_failure_mapping',      
      'entry_display_name',
      'entry_environment',
      'entry_endpoint',
      'button_save',
    ...
    $this->data['breadcrumbs'][] = array(
      ...
      'href' => $this->url->link('payment/duitku_varitel', 'token=' . $this->session->data['token'], 'SSL'),
      'separator' => ' :: '
    );

    $this->data['action'] = $this->url->link('payment/duitku_varitel', 'token=' . $this->session->data['token'], 'SSL');
    ...
    $inputs = array(            
      ...
      'duitku_varitel_order_status_id',
      'duitku_varitel_geo_zone_id',
      'duitku_varitel_sort_order',              
      'duitku_varitel_status',      
      'duitku_varitel_success_mapping',
      'duitku_varitel_failure_mapping',
      'duitku_varitel_challenge_mapping',
      'duitku_varitel_display_name',      
      'duitku_varitel_sanitization',      
    );
    ...
    $this->template = 'payment/duitku_varitel.tpl';
    $this->children = array(
      'common/header',
      'common/footer'
    );
  ...
  protected function validate() {
    ...
    if (!$this->user->hasPermission('modify', 'payment/duitku_varitel')) {
      $this->error['warning'] = $this->language->get('error_permission');
    }

    // check for empty values
    if (!$this->request->post['duitku_varitel_display_name']) {
      $this->error['display_name'] = $this->language->get('error_display_name');
    }       

    // check for empty values
    if (!$this->request->post['duitku_varitel_api_key']) {
