...
class ModelExtensionPaymentDuitkuDana extends Model {
  
  public function getMethod($address, $total) {
    
    $this->load->language('payment/duitku_dana');

    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('payment_duitku_dana_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

    if ($this->config->get('payment_duitku_dana_total') > 0 && $this->config->get('payment_duitku_dana_total') > $total) {
      $status = false;
    } elseif (!$this->config->get('payment_duitku_dana_geo_zone_id')) {
      $status = true;
    ...
    if ($status) {
      $method_data = array(
        'code'       => 'duitku_dana',
        'title'      => $this->config->get('payment_duitku_dana_display_name'),
        'sort_order' => $this->config->get('payment_duitku_dana_sort_order'),
        'terms'    => ''
      );
    }
