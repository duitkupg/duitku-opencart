...
class ModelExtensionPaymentDuitkuCC extends Model {
  
  public function getMethod($address, $total) {
    
    $this->load->language('payment/duitku_cc');

    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('payment_duitku_cc_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

    if ($this->config->get('payment_duitku_cc_total') > 0 && $this->config->get('payment_duitku_cc_total') > $total) {
      $status = false;
    } elseif (!$this->config->get('payment_duitku_cc_geo_zone_id')) {
      $status = true;
    ...
    if ($status) {
      $method_data = array(
        'code'       => 'duitku_cc',
        'title'      => $this->config->get('payment_duitku_cc_display_name'),
        'sort_order' => $this->config->get('payment_duitku_cc_sort_order'),
        'terms'    => ''
      );
    }
