...
class ModelExtensionPaymentDuitkuBniQris extends Model {
  
  public function getMethod($address, $total) {
    
    $this->load->language('payment/duitku_bni_qris');

    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('payment_duitku_bni_qris_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");

    if ($this->config->get('payment_duitku_bni_qris_total') > 0 && $this->config->get('payment_duitku_bni_qris_total') > $total) {
      $status = false;
    } elseif (!$this->config->get('payment_duitku_bni_qris_geo_zone_id')) {
      $status = true;
    ...
    if ($status) {
      $method_data = array(
        'code'       => 'duitku_bni_qris',
        'title'      => $this->config->get('payment_duitku_bni_qris_display_name'),
        'sort_order' => $this->config->get('payment_duitku_bni_qris_sort_order'),
        'terms'    => ''
      );
    }
