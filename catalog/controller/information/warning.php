...
class ControllerInformationWarning extends Controller {
    public function index() {        
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/information/warning')) { //if  file exists in your current template folder
            $this->template = $this->config->get('config_template') . '/template/information/warning'; //get it
        } else {
            $this->template = 'default/template/information/warning'; //or get the file from the default folder
        }
       ...
       /* $this->children = array(
        ...
        $this->response->setOutput($this->load->view('default/template/information/warning',$data));
    }
}
?>