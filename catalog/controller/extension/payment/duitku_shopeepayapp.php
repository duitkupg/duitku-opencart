...
class ControllerExtensionPaymentDuitkuShopeepayapp extends Controller {
  ...
  public function index() {
    ...
    $data['process_order'] = 'extension/payment/duitku_shopeepayapp/process_order';
  ...
  public function process_order() {    
    $this->load->model('extension/payment/duitku_shopeepayapp');
    $this->load->model('checkout/order');
    //$this->load->model('total/shipping');
    $this->load->language('extension/payment/duitku_shopeepayapp');
    ...
    //generate Signature
    $merchant_code = $this->config->get('duitku_shopeepayapp_merchant');
    $api_key = $this->config->get('duitku_shopeepayapp_api_key');
    $expired = $this->config->get('duitku_shopeepayapp_expired') != null ? $this->config->get('duitku_shopeepayapp_expired') : 60;
    ...
    $params = array(
          ...
          'returnUrl' => $this->url->link('extension/payment/duitku_shopeepayapp/landing_redir'),
          'callbackUrl' => $this->url->link('extension/payment/duitku_shopeepayapp/payment_notification'),
    ...
    try {
      ...
      $redirUrl = DuitkuCore_Web::getRedirectionUrl($this->config->get('duitku_shopeepayapp_endpoint'), $params);
   ...
   * http://[your shop’s homepage]/index.php?route=payment/duitku_shopeepayapp/payment_notification
   */
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode']) && isset($_GET['merchantOrderId']) && isset($_GET['reference']) && $_GET['resultCode'] == '01') {
	  ...
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_shopeepayapp_pending_mapping'), 'Duitku payment pending.');
      $redirUrl = $this->url->link('payment/duitku_shopeepayapp/failure');
      $this->response->redirect($redirUrl);
      ...
      $order_id = stripslashes($_GET['merchantOrderId']);
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_shopeepayapp_failure_mapping'), 'Duitku payment failed.');
      $redirUrl = $this->url->link('extension/payment/duitku_shopeepayapp/failure');
      $this->response->redirect($redirUrl);
  ...
  */
  ...
  public function payment_notification() {
    ...
    if (empty($_REQUEST['resultCode']) || empty($_REQUEST['merchantOrderId']) || empty($_REQUEST['reference'])) {
          throw new Exception(__('wrong query string please contact admin.', 'duitku_shopeepayapp'));
    }    
    ...
    $api_key = $this->config->get('duitku_shopeepayapp_api_key');
    $merchant_code = $this->config->get('duitku_shopeepayapp_merchant');    
    $endpoint = $this->config->get('duitku_shopeepayapp_endpoint');
    ...
    if ($order_info) {
        ...
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_shopeepayapp_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_shopeepayapp_failure_mapping'), 'Duitku payment failed.');
        }     
    }
