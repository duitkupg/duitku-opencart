...
class ControllerExtensionPaymentDuitkuShopeepayapp extends Controller {
  ...
  public function process_order() {    
	...
	$expired = $this->config->get('payment_duitku_shopeepayapp_expired') != null ? $this->config->get('payment_duitku_shopeepayapp_expired') : 60;
    ...
    try {  
      ...
      $result = DuitkuCore_Web::getRedirectionUrl($this->config->get('payment_duitku_shopeepayapp_endpoint'), $params);
  ...
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode']) && isset($_GET['merchantOrderId']) && isset($_GET['reference']) && $_GET['resultCode'] == '01') {
      ...
      $order_id = stripslashes($_GET['merchantOrderId']);
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_shopeepayapp_failure_mapping'), 'Duitku payment failed.');
  ...
  */
  ...
  public function payment_notification() {
    ...
    if ($current_status_id == $this->config->get('payment_duitku_shopeepayapp_success_mapping')){
      header("HTTP/1.1 200");
    ...
    if ($order_info) {
      $this->log->write("Callback Recieved : " . json_encode($_REQUEST, JSON_PRETTY_PRINT));
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_shopeepayapp_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_shopeepayapp_failure_mapping'), 'Duitku payment failed.');
        }     
    }
