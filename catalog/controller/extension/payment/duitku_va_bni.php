...
class ControllerExtensionPaymentDuitkuVABni extends Controller {
  ...
  public function index() {
    ...
    $data['process_order'] = 'extension/payment/duitku_va_bni/process_order';
  ...
  public function process_order() {    
    $this->load->model('extension/payment/duitku_va_bni');
    ...
    //generate Signature
    $merchant_code = $this->config->get('payment_duitku_va_bni_merchant');
    $api_key = $this->config->get('payment_duitku_va_bni_api_key');
	$expired = $this->config->get('payment_duitku_va_bni_expired') != null ? $this->config->get('payment_duitku_va_bni_expired') : 1440;
...
$shipping_address = array(
    ...
    $params = array(
      ...
      'customerVaName' => $order_info['payment_firstname'] . " " . $order_info['payment_lastname'],
          ...
          'returnUrl' => $this->url->link('extension/payment/duitku_va_bni/landing_redir'),
          'callbackUrl' => $this->url->link('extension/payment/duitku_va_bni/payment_notification'),
...
$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_va_bni_pending_mapping'), 'Duitku payment pending.');
      if ($this->config->get('payment_duitku_va_bni_environment') == 'Production'){
      ...
      $redirUrl = $this->url->link('extension/payment/duitku_va_bni/failure');
      $this->response->setOutput($redirUrl);
    }
  }
   ...
   * http://[your shop’s homepage]/index.php?route=payment/duitku_va_bni/payment_notification
   */
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode'], $_GET['merchantOrderId'], $_GET['reference'])) {
        ...
        if ($resultCode === '00') {
            ...
            // $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_va_bni_pending_mapping'), 'Duitku payment pending.');
            $redirUrl = $this->url->link('extension/payment/duitku_va_bni/pending');

        } else {
            // Failed/denied
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_va_bni_failure_mapping'), 'Duitku payment failed.');
            $redirUrl = $this->url->link('extension/payment/duitku_va_bni/failure');
        }
  ...
  */
  ...
  public function payment_notification() {
    ...
    if ($current_status_id == $this->config->get('payment_duitku_va_bni_success_mapping')){
      header("HTTP/1.1 200");
    ...
    if ($order_info) {
      try {
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key, $this->log)) {
          $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_va_bni_success_mapping'), 'Duitku payment successful.');    
        } else {
          $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_va_bni_failure_mapping'), 'Duitku payment failed.');
        } 
