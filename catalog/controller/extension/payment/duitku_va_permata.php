...
$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_va_permata_pending_mapping'), 'Duitku payment pending.');
      if ($this->config->get('payment_duitku_va_permata_environment') == 'Production'){
  ...
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode'], $_GET['merchantOrderId'], $_GET['reference'])) {
        ...
        if ($resultCode === '00') {
            ...
            // $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_va_permata_pending_mapping'), 'Duitku payment pending.');
            $redirUrl = $this->url->link('extension/payment/duitku_va_permata/pending');
  ...
  */
  ...
  public function payment_notification() {
    ...
    if ($current_status_id == $this->config->get('payment_duitku_va_permata_success_mapping')){
      header("HTTP/1.1 200");
