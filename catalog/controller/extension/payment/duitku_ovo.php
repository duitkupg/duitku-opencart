...
class ControllerExtensionPaymentDuitkuOvo extends Controller {
  ...
  public function process_order() {    
    ...
    try {     	  
      $redirUrl = Duitku_VtWeb::getRedirectionUrl($this->config->get('duitku_ovo_endpoint'), $params);
  ...
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode']) && isset($_GET['merchantOrderId']) && isset($_GET['reference']) && $_GET['resultCode'] == '01') {
	  ...
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_ovo_pending_mapping'), 'Duitku payment pending.');
      ...
      $order_id = stripslashes($_GET['merchantOrderId']);
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_ovo_failure_mapping'), 'Duitku payment failed.');
  ...
  public function payment_notification() {
    ...
    if ($order_info) {
        ...
        if ($status == '00' && Duitku_VtWeb::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_ovo_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_ovo_failure_mapping'), 'Duitku payment failed.');
        }     
    }
