...
class ControllerExtensionPaymentDuitkuCC extends Controller {
  ...
  public function process_order() {    
    ...
    try {     
      $redirUrl = Duitku_CoreWeb::getRedirectionUrl($this->config->get('payment_duitku_cc_endpoint'), $params);
  ...
  public function payment_notification() {
    ...
    if ($order_info) {
        ...
        if ($status == '00' && Duitku_CoreWeb::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_cc_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_cc_failure_mapping'), 'Duitku payment failed.');
        }     
    }
