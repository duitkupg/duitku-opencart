...
class ControllerExtensionPaymentDuitkuVARitel extends Controller {
  ...
  public function index() {
    ...
    $data['process_order'] = 'extension/payment/duitku_va_ritel/process_order';
  ...
  public function process_order() {    
    $this->load->model('extension/payment/duitku_va_ritel');
    ...
    //generate Signature
    $merchant_code = $this->config->get('duitku_va_ritel_merchant');
    $api_key = $this->config->get('duitku_va_ritel_api_key');
	$expired = $this->config->get('duitku_va_ritel_expired') != null ? $this->config->get('duitku_va_ritel_expired') : 1440;
    ...
    try {
      ...
      $redirUrl = DuitkuCore_Web::getRedirectionUrl($this->config->get('duitku_va_ritel_endpoint'), $params);
   ...
   * http://[your shop’s homepage]/index.php?route=payment/duitku_va_ritel/payment_notification
   */
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode']) && isset($_GET['merchantOrderId']) && isset($_GET['reference']) && $_GET['resultCode'] == '01') {
	  ...
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_va_ritel_pending_mapping'), 'Duitku payment pending.');
      $redirUrl = $this->url->link('extension/payment/duitku_va_ritel/failure');
      $this->response->redirect($redirUrl);
      ...
      $order_id = stripslashes($_GET['merchantOrderId']);
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_va_ritel_failure_mapping'), 'Duitku payment failed.');
      $redirUrl = $this->url->link('extension/payment/duitku_va_ritel/failure');
      $this->response->redirect($redirUrl);
  ...
  */
  ...
  public function payment_notification() {
    ...
    if (empty($_REQUEST['resultCode']) || empty($_REQUEST['merchantOrderId']) || empty($_REQUEST['reference'])) {
          throw new Exception(__('wrong query string please contact admin.', 'duitku_va_ritel'));
    }    
    ...
    $api_key = $this->config->get('duitku_va_ritel_api_key');
    $merchant_code = $this->config->get('duitku_va_ritel_merchant');    
    $endpoint = $this->config->get('duitku_va_ritel_endpoint');
    ...
    if ($order_info) {
        ...
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_va_ritel_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_va_ritel_failure_mapping'), 'Duitku payment failed.');
        }     
    }
