...
class ControllerExtensionPaymentDuitkuJeniusPay extends Controller {
  ...
  public function index() {
    ...
    $data['process_order'] = 'extension/payment/duitku_jenius_pay/process_order';
  ...
  public function process_order() {    
    ...
    //generate Signature
    $merchant_code = $this->config->get('duitku_jenius_pay_merchant');
    $api_key = $this->config->get('duitku_jenius_pay_api_key');
	$expired = $this->config->get('duitku_jenius_pay_expired') != null ? $this->config->get('duitku_jenius_pay_expired') : 1440;
    ...
    $params = array(
          ...
          'returnUrl' => $this->url->link('extension/payment/duitku_jenius_pay/landing_redir'),
          'callbackUrl' => $this->url->link('extension/payment/duitku_jenius_pay/payment_notification'),
    ...
    try {
      ...
      $redirUrl = DuitkuCore_Web::getRedirectionUrl($this->config->get('duitku_jenius_pay_endpoint'), $params);
   ...
   * http://[your shop’s homepage]/index.php?route=payment/duitku_jenius_pay/payment_notification
   */
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode']) && isset($_GET['merchantOrderId']) && isset($_GET['reference']) && $_GET['resultCode'] == '01') {
	  ...
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_jenius_pay_pending_mapping'), 'Duitku payment pending.');
      $redirUrl = $this->url->link('payment/duitku_jenius_pay/failure');
      $this->response->redirect($redirUrl);
      ...
      $order_id = stripslashes($_GET['merchantOrderId']);
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_jenius_pay_failure_mapping'), 'Duitku payment failed.');
  ...
  */
  ...
  public function payment_notification() {
    ...
    $api_key = $this->config->get('duitku_jenius_pay_api_key');
    $merchant_code = $this->config->get('duitku_jenius_pay_merchant');    
    $endpoint = $this->config->get('duitku_jenius_pay_endpoint');
    ...
    if ($order_info) {
        ...
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_jenius_pay_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_jenius_pay_failure_mapping'), 'Duitku payment failed.');
        }     
    }
