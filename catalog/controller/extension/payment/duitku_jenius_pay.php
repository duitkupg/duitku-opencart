...
class ControllerExtensionPaymentDuitkuJeniusPay extends Controller {
  ...
  public function process_order() {    
	...
	$expired = $this->config->get('payment_duitku_jenius_pay_expired') != null ? $this->config->get('payment_duitku_jenius_pay_expired') : 10;
...
$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_jenius_pay_pending_mapping'), 'Duitku payment pending.');
      if ($this->config->get('payment_duitku_jenius_pay_environment') == 'Production'){
  ...
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode'], $_GET['merchantOrderId'], $_GET['reference'])) {
        ...
        if ($resultCode === '00') {
            ...
            // $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_jenius_pay_pending_mapping'), 'Duitku payment pending.');
            $redirUrl = $this->url->link('extension/payment/duitku_jenius_pay/pending');
