...
class ControllerExtensionPaymentDuitkuBriva extends Controller {
  ...
  public function process_order() {    
    ...
    //generate Signature
    $merchant_code = $this->config->get('payment_duitku_briva_merchant');
    $api_key = $this->config->get('payment_duitku_briva_api_key');
	$expired = $this->config->get('payment_duitku_briva_expired') != null ? $this->config->get('payment_duitku_briva_expired') : 1440;
    ...
    try {  
      ...
      $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_briva_pending_mapping'), 'Duitku payment pending.');
    	
      $this->log->write("Request : " . json_encode($params, JSON_PRETTY_PRINT) );		
      $redirUrl = DuitkuCore_Web::getRedirectionUrl($this->config->get('payment_duitku_briva_endpoint'), $params,  $this->log);
  ...
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode']) && isset($_GET['merchantOrderId']) && isset($_GET['reference']) && $_GET['resultCode'] == '01') {
      ...
      $order_id = stripslashes($_GET['merchantOrderId']);
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_briva_failure_mapping'), 'Duitku payment failed.');
      $redirUrl = $this->url->link('extension/payment/duitku_briva/failure');
      $this->response->redirect($redirUrl);
  ...
  */
  ...
  public function payment_notification() {
    ...
    $api_key = $this->config->get('payment_duitku_briva_api_key');
    $merchant_code = $this->config->get('payment_duitku_briva_merchant');    
    $endpoint = $this->config->get('payment_duitku_briva_endpoint');
    ...
    if ($current_status_id == $this->config->get('payment_duitku_briva_success_mapping')){
      header("HTTP/1.1 200");
    ...
    if ($order_info) {
        ...
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_briva_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_briva_failure_mapping'), 'Duitku payment failed.');
        }     
    }
