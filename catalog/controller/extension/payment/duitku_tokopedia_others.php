...
class ControllerExtensionPaymentDuitkuTokopediaOthers extends Controller {
  ...
  public function index() {
    ...
    $data['process_order'] = 'extension/payment/duitku_tokopedia_others/process_order';
  ...
  public function process_order() {    
    $this->load->model('extension/payment/duitku_tokopedia_others');
    ...
    //generate Signature
    $merchant_code = $this->config->get('payment_duitku_tokopedia_others_merchant');
    $api_key = $this->config->get('payment_duitku_tokopedia_others_api_key');
	$expired = $this->config->get('payment_duitku_tokopedia_others_expired') != null ? $this->config->get('payment_duitku_tokopedia_others_expired') : 60;
  ...
  $customerDetails = array(
    ...
    $params = array(
      ...
      'returnUrl' => $this->url->link('extension/payment/duitku_tokopedia_others/landing_redir'),
      'callbackUrl' => $this->url->link('extension/payment/duitku_tokopedia_others/payment_notification'),
    ...
    try {  
      ...
      //Disable mail function => Dashboard => Extensions => Events => disable 'mail_order_add'
	    $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_tokopedia_others_pending_mapping'), 'Duitku payment pending.');
      if ($this->config->get('payment_duitku_tokopedia_others_environment') == 'Production'){
      ...
      $redirUrl = $this->url->link('extension/payment/duitku_tokopedia_others/failure');
      $this->response->setOutput($redirUrl);
    }
  }
   ...
   * http://[your shop’s homepage]/index.php?route=payment/duitku_tokopedia_others/payment_notification
   */
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode'], $_GET['merchantOrderId'], $_GET['reference'])) {
        ...
        if ($resultCode === '00') {
            ...
            // $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_tokopedia_others_pending_mapping'), 'Duitku payment pending.');
            $redirUrl = $this->url->link('extension/payment/duitku_tokopedia_others/pending');

        } else {
            // Failed/denied
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_tokopedia_others_failure_mapping'), 'Duitku payment failed.');
            $redirUrl = $this->url->link('extension/payment/duitku_tokopedia_others/failure');
        }
  ...
  */
  ...
  public function payment_notification() {
    ...
    if ($current_status_id == $this->config->get('payment_duitku_tokopedia_others_success_mapping')){
      header("HTTP/1.1 200");
    ...
    try {
        if ($status == '00' && DuitkuCore_Web::validateTransaction($baseUrl, $merchant_code, $order_id, $reference, $api_key, $this->log)) {
          $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_tokopedia_others_success_mapping'), 'Duitku payment successful.');    
        } else {
          $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_tokopedia_others_failure_mapping'), 'Duitku payment failed.');
        } 
