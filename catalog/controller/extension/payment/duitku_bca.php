...
class ControllerExtensionPaymentDuitkuBCA extends Controller {
  ...
  public function process_order() {    
    ...
    try {     
      $redirUrl = Duitku_CoreWeb::getRedirectionUrl($this->config->get('payment_duitku_bca_endpoint'), $params);
  ...
  public function payment_notification() {
    ...
    if ($order_info) {
        ...
        if ($status == '00' && Duitku_CoreWeb::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_bca_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_bca_failure_mapping'), 'Duitku payment failed.');
        }     
    }
