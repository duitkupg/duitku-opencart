...
class ControllerExtensionPaymentDuitkuVABca extends Controller {
  ...
  public function process_order() {    
    ...
    //generate Signature
    $merchant_code = $this->config->get('payment_duitku_va_bca_merchant');
    $api_key = $this->config->get('payment_duitku_va_bca_api_key');
	  $expired = $this->config->get('payment_duitku_va_bca_expired') != null ? $this->config->get('payment_duitku_va_bca_expired') : 1440;
    ...
    try {     
      ...
      $redirUrl = DuitkuCore_Web::getRedirectionUrl($this->config->get('payment_duitku_va_bca_endpoint'), $params,  $this->log);
  ...
  */
  ...
  public function payment_notification() {
    ...
    $api_key = $this->config->get('payment_duitku_va_bca_api_key');
    $merchant_code = $this->config->get('payment_duitku_va_bca_merchant');    
    $endpoint = $this->config->get('payment_duitku_va_bca_endpoint');
    ...
    if ($current_status_id == $this->config->get('payment_duitku_va_bca_success_mapping')){
      header("HTTP/1.1 200");
    ...
    if ($order_info) {
      $this->log->write("Callback Recieved : " . json_encode($_REQUEST, JSON_PRETTY_PRINT));
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_va_bca_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_va_bca_failure_mapping'), 'Duitku payment failed.');
        }     
    }
