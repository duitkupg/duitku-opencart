...
class ControllerExtensionPaymentDuitkuBniQris extends Controller {
  ...
  public function index() {
    ...
    $data['process_order'] = 'extension/payment/duitku_bni_qris/process_order';
  ...
  public function process_order() {    
    $this->load->model('extension/payment/duitku_bni_qris');
    ...
    //generate Signature
    $merchant_code = $this->config->get('payment_duitku_bni_qris_merchant');
    $api_key = $this->config->get('payment_duitku_bni_qris_api_key');
	  $expired = $this->config->get('payment_duitku_bni_qris_expired') != null ? $this->config->get('payment_duitku_bni_qris_expired') : 60;
    ...
    $params = array(
          ...
          'returnUrl' => $this->url->link('extension/payment/duitku_bni_qris/landing_redir'),
          'callbackUrl' => $this->url->link('extension/payment/duitku_bni_qris/payment_notification'),
    ...
    try {  
      ...
      $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_bni_qris_pending_mapping'), 'Duitku payment pending.');
      			
      $redirUrl = DuitkuCore_Web::getRedirectionUrl($this->config->get('payment_duitku_bni_qris_endpoint'), $params,  $this->log);
      $this->response->setOutput($redirUrl);
    }
    catch (Exception $e) {
      $this->log->write('Error : ' . $e->getMessage());
      $redirUrl = $this->url->link('extension/payment/duitku_bni_qris/failure');
      $this->response->setOutput($redirUrl);
    }
  }
   ...
   * http://[your shop’s homepage]/index.php?route=payment/duitku_bni_qris/payment_notification
   */
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode']) && isset($_GET['merchantOrderId']) && isset($_GET['reference']) && $_GET['resultCode'] == '01') {
      ...
      $order_id = stripslashes($_GET['merchantOrderId']);
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_bni_qris_failure_mapping'), 'Duitku payment failed.');
      $redirUrl = $this->url->link('extension/payment/duitku_bni_qris/failure');
      $this->response->redirect($redirUrl);
  ...
  */
  ...
  public function payment_notification() {
    ...
    $api_key = $this->config->get('payment_duitku_bni_qris_api_key');
    $merchant_code = $this->config->get('payment_duitku_bni_qris_merchant');    
    $endpoint = $this->config->get('payment_duitku_bni_qris_endpoint');
    ...
    if ($current_status_id == $this->config->get('payment_duitku_bni_qris_success_mapping')){
      header("HTTP/1.1 200");
    ...
    if ($order_info) {
      try {
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key, $this->log)) {
          $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_bni_qris_success_mapping'), 'Duitku payment successful.');    
        } else {
          $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_bni_qris_failure_mapping'), 'Duitku payment failed.');
        } 
