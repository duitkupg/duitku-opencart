...
class ControllerExtensionPaymentDuitkuGudangVoucherQris extends Controller {
  ...
  public function index() {
    ...
    $data['process_order'] = 'extension/payment/duitku_gudang_voucher_qris/process_order';
  ...
  public function process_order() {    
    $this->load->model('extension/payment/duitku_gudang_voucher_qris');
    ...
    //generate Signature
    $merchant_code = $this->config->get('payment_duitku_gudang_voucher_qris_merchant');
    $api_key = $this->config->get('payment_duitku_gudang_voucher_qris_api_key');
	$expired = $this->config->get('payment_duitku_gudang_voucher_qris_expired') != null ? $this->config->get('payment_duitku_gudang_voucher_qris_expired') : 60;
    ...
    $params = array(
          ...
          'returnUrl' => $this->url->link('extension/payment/duitku_gudang_voucher_qris/landing_redir'),
          'callbackUrl' => $this->url->link('extension/payment/duitku_gudang_voucher_qris/payment_notification'),
    ...
    try {  
      ...
      $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_gudang_voucher_qris_pending_mapping'), 'Duitku payment pending.');
	
      $this->log->write("Request : " . json_encode($params, JSON_PRETTY_PRINT) );		
      $redirUrl = DuitkuCore_Web::getRedirectionUrl($this->config->get('payment_duitku_gudang_voucher_qris_endpoint'), $params,  $this->log);
   ...
   * http://[your shop’s homepage]/index.php?route=payment/duitku_gudang_voucher_qris/payment_notification
   */
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode']) && isset($_GET['merchantOrderId']) && isset($_GET['reference']) && $_GET['resultCode'] == '01') {
	  ...
	  // $this->cart->clear();
	  // $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_gudang_voucher_qris_pending_mapping'), 'Duitku payment pending.');
      $redirUrl = $this->url->link('payment/duitku_gudang_voucher_qris/failure');
      $this->response->redirect($redirUrl);
      ...
      $order_id = stripslashes($_GET['merchantOrderId']);
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_gudang_voucher_qris_failure_mapping'), 'Duitku payment failed.');
      $redirUrl = $this->url->link('extension/payment/duitku_gudang_voucher_qris/failure');
      $this->response->redirect($redirUrl);
  ...
  */
  ...
  public function payment_notification() {
    ...
    if (empty($_REQUEST['resultCode']) || empty($_REQUEST['merchantOrderId']) || empty($_REQUEST['reference'])) {
          throw new Exception(__('wrong query string please contact admin.', 'duitku_gudang_voucher_qris'));
    }    
    ...
    $api_key = $this->config->get('payment_duitku_gudang_voucher_qris_api_key');
    $merchant_code = $this->config->get('payment_duitku_gudang_voucher_qris_merchant');    
    $endpoint = $this->config->get('payment_duitku_gudang_voucher_qris_endpoint');
    ...
    if ($current_status_id == $this->config->get('payment_duitku_gudang_voucher_qris_success_mapping')){
      header("HTTP/1.1 200");
    ...
    if ($order_info) {
      $this->log->write("Callback Recieved : " . json_encode($_REQUEST, JSON_PRETTY_PRINT));
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_gudang_voucher_qris_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_gudang_voucher_qris_failure_mapping'), 'Duitku payment failed.');
        }     
    }
