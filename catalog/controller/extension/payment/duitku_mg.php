...
class ControllerExtensionPaymentDuitkuMG extends Controller {
  ...
  public function process_order() {    
    ...
    //generate Signature
    $merchant_code = $this->config->get('payment_duitku_mg_merchant');
    $api_key = $this->config->get('payment_duitku_mg_api_key');
    $cred_code = $this->config->get('payment_duitku_mg_credcode');
    ...
    try {
      ...
      $redirUrl = DuitkuCore_Web::getRedirectionUrl($this->config->get('payment_duitku_mg_endpoint'), $params);
  ...
  */
  ...
  public function payment_notification() {
    ...
    $cred_code = $this->config->get('payment_duitku_mg_credcode');
    $merchant_code = $this->config->get('payment_duitku_mg_merchant');    
    $endpoint = $this->config->get('payment_duitku_mg_endpoint');
    ...
    if ($order_info) {
        ...
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_mg_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_duitku_mg_failure_mapping'), 'Duitku payment failed.');
        }     
    }
