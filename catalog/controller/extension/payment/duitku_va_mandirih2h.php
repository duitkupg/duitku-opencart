...
class ControllerExtensionPaymentDuitkuVAMandirih2h extends Controller {
  ...
  public function index() {
    ...
    $data['process_order'] = 'extension/payment/duitku_va_mandirih2h/process_order';

    if(version_compare(VERSION, '2.2.0.0') < 0) {
      // CODE HERE IF LOWER
      if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/duitku.tpl')) {
        return $this->load->view($this->config->get('config_template') . '/template/extension/payment/duitku.tpl', $data);
      } else {
        return $this->load->view('default/template/extension/payment/duitku.tpl', $data);
      }
    } else {
      // CODE HERE IF HIGHER OR EQUAL
      return $this->load->view('extension/payment/duitku', $data);
    }  
  ...
  public function process_order() {    
    $this->load->model('extension/payment/duitku_va_mandirih2h');
    ...
    //generate Signature
    $merchant_code = $this->config->get('duitku_va_mandirih2h_merchant');
    $api_key = $this->config->get('duitku_va_mandirih2h_api_key');
	$expired = $this->config->get('duitku_va_mandirih2h_expired') != null ? $this->config->get('duitku_va_mandirih2h_expired') : 1440;
    ...
    $params = array(
          ...
          'returnUrl' => $this->url->link('extension/payment/duitku_va_mandirih2h/landing_redir'),
          'callbackUrl' => $this->url->link('extension/payment/duitku_va_mandirih2h/payment_notification'),
    ...
    try {
      ...
      $redirUrl = DuitkuCore_Web::getRedirectionUrl($this->config->get('duitku_va_mandirih2h_endpoint'), $params);
   ...
   * http://[your shop’s homepage]/index.php?route=payment/duitku_va_mandirih2h/payment_notification
   */
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode']) && isset($_GET['merchantOrderId']) && isset($_GET['reference']) && $_GET['resultCode'] == '01') {
	  ...
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_va_mandirih2h_pending_mapping'), 'Duitku payment pending.');
      $redirUrl = $this->url->link('payment/duitku_va_mandirih2h/failure');
      $this->response->redirect($redirUrl);
      ...
      $order_id = stripslashes($_GET['merchantOrderId']);
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_va_mandirih2h_failure_mapping'), 'Duitku payment failed.');
      $redirUrl = $this->url->link('extension/payment/duitku_va_mandirih2h/failure');
      $this->response->redirect($redirUrl);
  ...
  */
  ...
  public function payment_notification() {
    ...
    if (empty($_REQUEST['resultCode']) || empty($_REQUEST['merchantOrderId']) || empty($_REQUEST['reference'])) {
          throw new Exception(__('wrong query string please contact admin.', 'duitku_va_mandirih2h'));
    }    
    ...
    $api_key = $this->config->get('duitku_va_mandirih2h_api_key');
    $merchant_code = $this->config->get('duitku_va_mandirih2h_merchant');    
    $endpoint = $this->config->get('duitku_va_mandirih2h_endpoint');
    ...
    if ($order_info) {
        ...
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_va_mandirih2h_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_va_mandirih2h_failure_mapping'), 'Duitku payment failed.');
        }     
    }
