...
class ControllerPaymentDuitkuMG extends Controller {
  ...
  public function index() {
    ...
    $data['process_order'] = 'payment/duitku_mg/process_order';
  ...
  public function process_order() {    
    $this->load->model('payment/duitku_mg');
    $this->load->model('checkout/order');
    $this->load->model('total/shipping');
    $this->load->language('payment/duitku_mg');
    ...
    $cred_code = $this->config->get('duitku_mg_cred_code');

    //generate Signature
    $merchant_code = $this->config->get('duitku_mg_merchant');
    $api_key = $this->config->get('duitku_mg_api_key');
    ...
    $params = array(
          ...
          'signature' => $signature,
		      'expiryPeriod' => 1440,       
          'returnUrl' => $this->url->link('payment/duitku_mg/landing_redir'),
          'callbackUrl' => $this->url->link('payment/duitku_mg/payment_notification'),
    ...
    try {
      ...
      $redirUrl = DuitkuCore_Web::getRedirectionUrl($this->config->get('duitku_mg_endpoint'), $params);
   ...
   * http://[your shop’s homepage]/index.php?route=payment/duitku_mg/payment_notification
   */
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode']) && isset($_GET['merchantOrderId']) && isset($_GET['reference']) && $_GET['resultCode'] == '00') {
      ...
      // $redirUrl = $this->url->link('checkout/cart');
      $redirUrl = $this->url->link('payment/duitku_mg/failure');
      $this->response->redirect($redirUrl);
  ...
  */
  ...
  public function payment_notification() {
    ...
    $api_key = $this->config->get('duitku_mg_api_key');
    $merchant_code = $this->config->get('duitku_mg_merchant');    
    $endpoint = $this->config->get('duitku_mg_endpoint');
    ...
    if ($order_info) {
        ...
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_mg_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_mg_failure_mapping'), 'Duitku payment failed.');
        }     
    }
