...
class ControllerPaymentDuitkuDANA extends Controller {
  ...
  public function index() {
    ...
    $data['process_order'] = 'payment/duitku_dana/process_order';
  ...
  public function process_order() {    
    $this->load->model('payment/duitku_dana');
    $this->load->model('checkout/order');
    $this->load->model('total/shipping');
    $this->load->language('payment/duitku_dana');
    ...
    //generate Signature
    $merchant_code = $this->config->get('duitku_dana_merchant');
    $api_key = $this->config->get('duitku_dana_api_key');
    $expired = $this->config->get('duitku_dana_expired') != null ? $this->config->get('duitku_dana_expired') : 1440;
    ...
    $params = array(
          ...
          'returnUrl' => $this->url->link('payment/duitku_dana/landing_redir'),
          'callbackUrl' => $this->url->link('payment/duitku_dana/payment_notification'),
    ...
    try {
      ...
      $redirUrl = DuitkuCore_Web::getRedirectionUrl($this->config->get('duitku_dana_endpoint'), $params);
  ...
  */
  ...
  public function payment_notification() {
    ...
    $api_key = $this->config->get('duitku_dana_api_key');
    $merchant_code = $this->config->get('duitku_dana_merchant');    
    $endpoint = $this->config->get('duitku_dana_endpoint');
    ...
    if ($order_info) {
        ...
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_dana_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_dana_failure_mapping'), 'Duitku payment failed.');
        }     
    }
