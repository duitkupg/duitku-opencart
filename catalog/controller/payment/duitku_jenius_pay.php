...
class ControllerPaymentDuitkuJeniusPay extends Controller {
  ...
  public function index() {
    ...
    $data['process_order'] = 'payment/duitku_jenius_pay/process_order';
  ...
  public function process_order() {    
    $this->load->model('payment/duitku_jenius_pay');
    $this->load->model('checkout/order');
    $this->load->model('total/shipping');
    $this->load->language('payment/duitku_jenius_pay');
    ...
    $params = array(
          ...
          'returnUrl' => $this->url->link('payment/duitku_jenius_pay/landing_redir'),
          'callbackUrl' => $this->url->link('payment/duitku_jenius_pay/payment_notification'),
    ...
    $data['checkout_url'] = $this->url->link('checkout/cart');
     ...
     if(version_compare(VERSION, '2.2.0.0') < 0) {
      ...
      if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/duitku_checkout_failure.tpl')) {
        $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/duitku_checkout_failure.tpl', $data));
      } else {
        $this->response->setOutput($this->load->view('default/template/payment/duitku_checkout_failure', $data));
      }
    } else {
      // CODE HERE IF HIGHER OR EQUAL
      $this->response->setOutput($this->load->view('payment/duitku_checkout_failure', $data));
    }        
  }
