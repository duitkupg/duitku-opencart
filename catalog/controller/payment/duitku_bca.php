...
class ControllerPaymentDuitkuBCA extends Controller {
  ...
  public function index() {
    ...
    $data['process_order'] = 'payment/duitku_bca/process_order';
  ...
  public function process_order() {    
    $this->load->model('payment/duitku_bca');
    ...
    $params = array(
          ...
          'signature' => $signature,          
          'returnUrl' => $this->url->link('payment/duitku_bca/landing_redir'),
          'callbackUrl' => $this->url->link('payment/duitku_bca/payment_notification'),
    );         

    try {     
      $redirUrl = Duitku_VtWeb::getRedirectionUrl($this->config->get('duitku_bca_endpoint'), $params);
  ...
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode']) && isset($_GET['merchantOrderId']) && isset($_GET['reference']) && $_GET['resultCode'] == '00') {
      ...
      // $redirUrl = $this->url->link('checkout/cart');
      $redirUrl = $this->url->link('payment/duitku_bca/failure');
      $this->response->redirect($redirUrl);
