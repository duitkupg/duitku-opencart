...
class ControllerPaymentDuitkuCC extends Controller {
  ...
  public function index() {
    ...
    $this->data['process_order'] = $this->url->link('payment/duitku_cc/process_order');
  ...
  */
  ...
  public function payment_notification() {    
    ...
    if (empty($_REQUEST['resultCode']) || empty($_REQUEST['merchantOrderId']) || empty($_REQUEST['reference'])) {
          throw new Exception(__('wrong query string please contact admin.', 'duitku_cc'));
    }    
    ...
    if ($order_info) {        
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
          $order_status_id = $this->config->get('duitku_cc_success_mapping');          
        } else {
          $order_status_id = $this->config->get('duitku_cc_failure_mapping');        
        }     
