...
class ControllerPaymentDuitkuCC extends Controller {
  ...
  public function index() {
    ...
    $data['process_order'] = 'payment/duitku_cc/process_order';
  ...
  public function process_order() {    
    $this->load->model('payment/duitku_cc');
    $this->load->model('checkout/order');
    $this->load->model('total/shipping');
    $this->load->language('payment/duitku_cc');
    ...
    //generate Signature
    $merchant_code = $this->config->get('duitku_cc_merchant');
    $api_key = $this->config->get('duitku_cc_api_key');
    ...
    $params = array(
          ...
          'signature' => $signature,
		  'expiryPeriod' => 1440,       
          'returnUrl' => $this->url->link('payment/duitku_cc/landing_redir'),
          'callbackUrl' => $this->url->link('payment/duitku_cc/payment_notification'),
    ...
    try {
      ...
      $redirUrl = DuitkuCore_Web::getRedirectionUrl($this->config->get('duitku_cc_endpoint'), $params);
   ...
   * http://[your shop’s homepage]/index.php?route=payment/duitku_cc/payment_notification
   */
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode']) && isset($_GET['merchantOrderId']) && isset($_GET['reference']) && $_GET['resultCode'] == '00') {
      ...
      // $redirUrl = $this->url->link('checkout/cart');
      $redirUrl = $this->url->link('payment/duitku_cc/failure');
      $this->response->redirect($redirUrl);
  ...
  */
  ...
  public function payment_notification() {
    ...
    $api_key = $this->config->get('duitku_cc_api_key');
    $merchant_code = $this->config->get('duitku_cc_merchant');    
    $endpoint = $this->config->get('duitku_cc_endpoint');
    ...
    if ($order_info) {
        ...
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_cc_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_cc_failure_mapping'), 'Duitku payment failed.');
        }     
    }
