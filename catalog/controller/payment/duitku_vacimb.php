...
class ControllerPaymentDuitkuVacimb extends Controller {
  ...
  public function index() {
    ...
    $this->data['process_order'] = $this->url->link('payment/duitku_vacimb/process_order');
  ...
  public function process_order() {    
    $this->load->model('payment/duitku_vacimb');
  ...
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode']) && isset($_GET['merchantOrderId']) && isset($_GET['reference']) && $_GET['resultCode'] == '00') {
      ...
      // $redirUrl = $this->url->link('checkout/cart');
      $redirUrl = $this->url->link('payment/duitku_vacimb/failure');
      $this->response->redirect($redirUrl);
  ...
  */
  ...
  public function payment_notification() {    
    ...
    if (empty($_REQUEST['resultCode']) || empty($_REQUEST['merchantOrderId']) || empty($_REQUEST['reference'])) {
          throw new Exception(__('wrong query string please contact admin.', 'duitku_vacimb'));
    }    
    ...
    $api_key = $this->config->get('duitku_vacimb_api_key');
    $merchant_code = $this->config->get('duitku_vacimb_merchant');    
    $endpoint = $this->config->get('duitku_vacimb_endpoint');
