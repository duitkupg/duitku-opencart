...
class ControllerPaymentDuitkuBriva extends Controller {
  ...
  public function index() {
    ...
    $this->data['process_order'] = $this->url->link('payment/duitku_briva/process_order');
  ...
  public function payment_notification() {    
    ...
    if ($order_info) {        
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
          $order_status_id = $this->config->get('duitku_briva_success_mapping');          
        } else {
          $order_status_id = $this->config->get('duitku_briva_failure_mapping');       
        }     
