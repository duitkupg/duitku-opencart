...
class ControllerPaymentDuitkuVAPermata extends Controller {
  ...
  public function index() {
    ...
    $data['process_order'] = 'payment/duitku_va_permata/process_order';
  ...
  public function process_order() {    
    $this->load->model('payment/duitku_va_permata');
    ...
    //generate Signature
    $merchant_code = $this->config->get('duitku_va_permata_merchant');
    $api_key = $this->config->get('duitku_va_permata_api_key');
	$expired = $this->config->get('duitku_va_permata_expired') != null ? $this->config->get('duitku_va_permata_expired') : 1440;
    ...
    $params = array(
          ...
          'returnUrl' => $this->url->link('payment/duitku_va_permata/landing_redir'),
          'callbackUrl' => $this->url->link('payment/duitku_va_permata/payment_notification'),
    ...
    try {
      ...
      $redirUrl = DuitkuCore_Web::getRedirectionUrl($this->config->get('duitku_va_permata_endpoint'), $params);
  ...
  public function landing_redir() {    
    ...
    if (isset($_GET['resultCode']) && isset($_GET['merchantOrderId']) && isset($_GET['reference']) && $_GET['resultCode'] == '01') {
	  ...
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_va_permata_pending_mapping'), 'Duitku payment pending.');
      $redirUrl = $this->url->link('payment/duitku_va_permata/failure');
      $this->response->redirect($redirUrl);
      ...
      $order_id = stripslashes($_GET['merchantOrderId']);
	  $this->cart->clear();
	  $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_va_permata_failure_mapping'), 'Duitku payment failed.');
      $redirUrl = $this->url->link('payment/duitku_va_permata/failure');
      $this->response->redirect($redirUrl);
  ...
  */
  ...
  public function payment_notification() {
    ...
    $api_key = $this->config->get('duitku_va_permata_api_key');
    $merchant_code = $this->config->get('duitku_va_permata_merchant');    
    $endpoint = $this->config->get('duitku_va_permata_endpoint');
    ...
    if ($order_info) {
        ...
        if ($status == '00' && DuitkuCore_Web::validateTransaction($endpoint, $merchant_code, $order_id, $reference, $api_key)) {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_va_permata_success_mapping'), 'Duitku payment successful.');    
        } else {
            $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('duitku_va_permata_failure_mapping'), 'Duitku payment failed.');
        }     
    }
